...
function wpa_skiplink_css() {
	...
	if ( 'true' === $use_defaults ) {
		...
		if ( '' !== get_option( 'asl_styles' ) ) {
			$styles = wp_kses_filter_nohtml( get_option( 'asl_styles' ) );
		} else {
			$focus   = wp_kses_filter_nohtml( get_option( 'asl_styles_focus' ) );
			$passive = wp_kses_filter_nohtml( get_option( 'asl_styles_passive' ) );
		}
	}
	$styles = '';
